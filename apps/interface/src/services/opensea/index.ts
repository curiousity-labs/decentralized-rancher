...
export const retrieveERC721 = async (mappedNfts: ERC721Data[], account: string): Promise<OpenseaAsset[]> => {
  ...
  const response = await axios.get<OpenseaResponse<OpenseaAsset>>(`${OPENSEA_TESTNET_BASE_URL}/assets?${params}`)
